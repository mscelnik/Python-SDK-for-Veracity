    ...
    async def test_create_container(self, api):
        ...
        with patch_response(api.session, "post", 202, text=b"MOCK_GUID") as mockpost:
            ...
            mockpost.assert_called_with(
                "https://api.veracity.com/veracity/datafabric/provisioning/api/1/container", expected_body
            )
            assert data == "MOCK_GUID"

    @pytest.mark.asyncio
    async def test_create_container_context(self, api_context):
        """ Creating a new container has no exceptions (with context manager).
        """
        with patch_response(api_context.session, "post", 202, text=b"MOCK_GUID") as mockpost:
            data = await api_context.create_container(
                "mycontainer",
                "My Container",
                description="My new container",
                region="westeurope",
...
class TestProvisionAPIContext(object):

    @pytest.fixture(scope="function", autouse=True)
    async def MockSession(self):
        # Mock out the aiohttp session for unit testing.
        with mock.patch("veracity_platform.base.ClientSession", autospec=True) as mocked:
            yield mocked
